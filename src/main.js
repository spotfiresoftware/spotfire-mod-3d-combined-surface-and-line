...
Spotfire.initialize(async (mod) => {
    ...
    let validateAxes = function() {
        ...
        valid = valid && validateAxisExpression("Z");
    ...
    //   if not found it will create one
    let getGroupItem = function(trellisItem, groupBy, layerType, mode) {
    ...
    let processRows = async function() {
        ...
        rows.forEach(function(row) {
            ...
            let thisGroupItem = getGroupItem(thisTrellisItem, object.groupBy, object.layerType, object.mode);
            thisGroupItem.name = object.name;
            thisGroupItem.label = object.label;
    ...
    let rowToObject = function(row) {
        ...
        object.orderBy = axisHasExpression("Order By") ? row.continuous("Order By").value() : null;
        object.layerType = row.categorical("Layer Type").formattedValue();
        object.mode = axisHasExpression("Display Mode") ? row.categorical("Display Mode").formattedValue() : null;
        ...
        object.symbol = axisHasExpression("Symbol") ? row.categorical("Symbol").formattedValue() : null;
        ...
        object.z = row.continuous("Z").value();
    ...
    let drawViz = async function(trellisItemMap) {  
        ...
        let configuration = {
            ...
            marking: marking,
            dark: dark,
            hasDisplayModeData: axisHasExpression("Display Mode"),
            hasSizeData: axisHasExpression("Size"),
            hasSymbolData: axisHasExpression("Symbol"),
            lineWidth: modConfig.plotly.scatter3d.lineWidth,
            mode: modConfig.plotly.scatter3d.mode,
            markerLineEnabled: modConfig.plotly.scatter3d.markerLineEnabled,
        ...
        for(let key in trellisItemMap) {
            ...
            if(diagram == null) {
                ...
                diagram = new Plotly3DCombinedDiagram(canvasElem, actions);
    ...
    const reader = mod.createReader(
        ...
        mod.visualization.axis("Display Mode"),
        mod.visualization.axis("Name"),
        mod.visualization.axis("Label"),
        mod.visualization.axis("Symbol"),
    ...
    async function render(colorView, trellisByView, groupByView, orderByView, layerTypeView, displayModeView, nameView, labelView, symbolView, sizeView, xView, yView, zView,
            dataView, windowSize) {
        ...
        // Copy the axes
        let axesArr = [colorView, trellisByView, groupByView, orderByView, layerTypeView, displayModeView, nameView, labelView, symbolView, sizeView, xView, yView, zView];
        for(let thisAxis of axesArr) {
            axes[thisAxis.name] = thisAxis;
        }

        // Set marking flag based on the marking configuration
